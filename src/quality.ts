 ...
 */
export function checkPlausibility(input: PlausibilityInput): DataQualityFlag[] {
  ...
  if (hours < MIN_MEANINGFUL_HOURS) {
    flags.push({
      ...
      severity: 'warn',
      message:
        `Only ${Math.round(hours).toLocaleString()} hours worked. A single recordable case would ` +
  ...
  if (employees !== undefined && Number.isFinite(employees) && employees > 0) {
    ...
    if (perEmployee < MIN_HOURS_PER_EMPLOYEE || perEmployee > MAX_HOURS_PER_EMPLOYEE) {
      flags.push({
        ...
        severity: 'error',
        message:
          `${Math.round(perEmployee).toLocaleString()} hours per employee per year is outside the ` +
    ...
    if (totalCases !== undefined && Number.isFinite(totalCases) && totalCases > employees) {
      flags.push({
        ...
        severity: 'warn',
        message:
          `${totalCases} recordable cases reported for ${employees} average employees. ` +
...
export function isPublishable(flags: DataQualityFlag[]): boolean {
