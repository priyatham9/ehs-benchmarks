const PERCENTILE_KEYS = ['p10', 'p25', 'p50', 'p75', 'p90', 'p95', 'p99'];
...
export function sizeBandFor(employees) {
...
export function percentile(sortedAsc, p) {
    if (sortedAsc.length === 0)
        throw new RangeError('Cannot take a percentile of an empty array.');
    if (p < 0 || p > 100)
        throw new RangeError(`Percentile must be 0-100 (received ${p}).`);
...
export function buildDistribution(rates, meta) {
    if (rates.length === 0) {
 ...
 */
export function rankAgainst(rate, dist) {
    ...
    if (rate === 0 && dist.zeroRate > 0) {
        ...
        const points = [
            [0, dist.zeroRate * 100],
            ...PERCENTILE_KEYS.map((k) => [dist.percentiles[k], PERCENTILE_VALUES[k]]).filter(([value, pct]) => value > 0 && pct > dist.zeroRate * 100),
        ];
